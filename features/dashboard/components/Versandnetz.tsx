...
function SendungChip({
  ...
  const pe = sendung.packungseinheit ? PE_SHORT[sendung.packungseinheit] : null;
...
export default function Versandnetz({
  ...
  const goToday = useCallback(() => setCurrentDate(dayjs()), []);
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 h-11 bg-white border-b border-slate-200 shrink-0">
          ...
          <AnimatePresence>
            {viewMode === "week" && (
              <motion.span
                ...
                transition={{ duration: 0.15 }}
                className="text-[10px] font-medium text-slate-400 bg-slate-100 rounded px-1.5 py-0.5 shrink-0"
              >
                KW {weekNum}
              </motion.span>
            )}
        ...
        <div className="flex items-center gap-1 text-[11px] text-slate-400 shrink-0">
          ...
          <span className="font-mono font-medium">{now.format("HH:mm")}</span>
        </div>
      ...
      <div className="flex-1 overflow-auto">
        <AnimatePresence mode="wait">
          <motion.div
            key={`grid-${viewMode}-${visibleDays[0]?.format("YYYYMMDD")}`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
                      ...
                      ${viewMode === "day" ? "min-w-80" : "min-w-36"}`}
                        >
                          <div className="flex items-center gap-1.5">
                            ...
                            {isToday && (
                              <span className="ml-auto flex items-center gap-1 text-[9px] font-semibold bg-white/20 text-white rounded px-1.5 py-0.5">
